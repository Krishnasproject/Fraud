...
def register():
    if request.method == 'POST':
        ...
        if db.session.query(User).filter_by(username=username).first() or not whatsapp_number or not email:
...
def predict():
    try:
        ...
        if isinstance(transaction_type, str):
            data['TransactionType'] = {'Transfer': 0, 'Withdrawal': 1, 'Deposit': 2}.get(transaction_type)
